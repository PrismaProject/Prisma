...
import me.Prisma.utils.Debug.thePlayerisBlocking
import me.Prisma.utils.PacketUtils
...
class NoSlow : Module() {
    ...
    private val     modeValue = ListValue("PacketMode", arrayOf(
        "None",
        "NoCheatPlus",
        "NoPacket",
        "Hypixel",
    ...
    fun onMotion(event: MotionEvent) {
        ...
        val C07 = classProvider.createCPacketPlayerDigging(ICPacketPlayerDigging.WAction.RELEASE_USE_ITEM, WBlockPos(-1,-1,-1),EnumFacing.DOWN as IEnumFacing)
        val C08 = classProvider.createCPacketPlayerBlockPlacement(mc.thePlayer!!.inventory.currentItem as IItemStack)
        val C03 = classProvider.createCPacketPlayer(mc.thePlayer!!.inventory.getCurrentItemInHand() as Boolean)
        val C08WBlockPos = classProvider.createCPacketPlayerBlockPlacement(WBlockPos(-1, -1, -1), 255, mc.thePlayer!!.inventory.currentItem as IItemStack, 0f, 0f, 0f)
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "aac4" ->{
                    if(mc.thePlayer!!.ticksExisted % 3 == 0) {
                        this.sendPacket(event,SendC07 = true,SendC08 = false,Delay = false,DelayValue = 0 , onGround = false ,Hypixel = false)
                    } else {
                        this.sendPacket(event,SendC07 = false,SendC08 = true,Delay = false,DelayValue = 0 , onGround = false ,Hypixel = false)
                    }
                }
                "packet" ->{
                    this.sendPacket(event,SendC07 = true,SendC08 = true,Delay = false , DelayValue = 0 , onGround = false ,Hypixel = false)
                }
                "test5" -> {
                    mc.netHandler.addToSendQueue(classProvider.createCPacketHeldItemChange(mc.thePlayer!!.inventory.getCurrentItemInHand() as Int))
                    mc.netHandler.addToSendQueue(C07)
                }
                "test6" -> {
                    mc.netHandler.addToSendQueue(classProvider.createCPacketHeldItemChange(mc.thePlayer!!.inventory.getCurrentItemInHand() as Int))
                    mc.netHandler.addToSendQueue(C08WBlockPos)
                }
                "test7" -> {
                    mc.netHandler.addToSendQueue(classProvider.createCPacketHeldItemChange(mc.thePlayer!!.inventory.getCurrentItemInHand() as Int))
                    mc.netHandler.addToSendQueue(C03)
                }
                "test2" -> {
                   mc.netHandler.addToSendQueue(classProvider.createCPacketHeldItemChange(mc.thePlayer!!.inventory.getCurrentItemInHand() as Int))
                ...
                "hypixel" -> {
                    this.sendPacket(event, true, true, false, 0, false, false)
                }
                "liquidbounce" -> {
