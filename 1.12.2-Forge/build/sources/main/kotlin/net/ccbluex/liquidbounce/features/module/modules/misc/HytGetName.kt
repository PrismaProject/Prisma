...
class HytGetName : Module() {
    ...
    private val ClientNameValue = TextValue("ClientName", "Prisma“")
    ...
    fun onPacket(event: PacketEvent) { //Trim 去除名字后面的空字符串
        val clientname = ClientNameValue
                ...
                "4v4/1v1", "32/64" -> {
                    ...
                    if (matcher.find()) {
                        ...
                        if (name != "") {
                            ...
                            ClientUtils.displayChatMessage(clientname.toString() +  "§8[§c§l提醒您§8]§c§d添加无敌人：" + name)
                            Thread {
                                try {
                                    Thread.sleep(5000)
                                    LiquidBounce.fileManager.friendsConfig.removeFriend(name)
                                    ClientUtils.displayChatMessage(clientname.toString() + "§8[§c§l提醒您§8]§c§d删除无敌人：" + name)
