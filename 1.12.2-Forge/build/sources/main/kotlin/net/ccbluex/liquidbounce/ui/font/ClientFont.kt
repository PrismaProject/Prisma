...
import net.ccbluex.liquidbounce.utils.render.shader.shaders.RainbowFontShaderLow
...
class ClientFont(font: Font) : FontRenderer(Minecraft.getMinecraft().gameSettings,
        ResourceLocation("textures/font/ascii.png"), if (ClassUtils.hasForge()) null else Minecraft.getMinecraft().textureManager, false) {

    var defaultFont = ClientUnicodeFont(font)
    private var boldFont = ClientUnicodeFont(font.deriveFont(Font.BOLD))
    private var italicFont = ClientUnicodeFont(font.deriveFont(Font.ITALIC))
    private var boldItalicFont = ClientUnicodeFont(font.deriveFont(Font.BOLD or Font.ITALIC))
    ...
    override fun drawString(text: String, x: Float, y: Float, color: Int, shadow: Boolean): Int {
        ...
        val rainbow = RainbowFontShaderLow.INSTANCE.isInUse

        if (shadow) {
            glUseProgram(0)
            drawText(currentText, x + 0.5F, currY + 1F, Color(0, 0, 0, 150).rgb, true)
        }
    ...
    private fun drawText(text: String?, x: Float, y: Float, color: Int, ignoreColor: Boolean, rainbow: Boolean = false): Int {
        ...
        val rainbowShaderId = RainbowFontShaderLow.INSTANCE.programId
